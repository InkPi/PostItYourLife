class PostItController < ApplicationController
  ...
  before_action :set_post_it, only: [:show, :create, :update, :destroy]
  ...
  def create
    ...
    if @post_it.save
      render json: @post_it, status: :created, location: @post_it
    else
  ...
  private
    ...
    def post_it_params
      params.require(:post_it).permit(:title, :content)
    end
end
